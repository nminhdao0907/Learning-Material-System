 ...
 * @author dangn
 */
public class ExtraLessonDAO extends BaseDAO {

    public ArrayList<ExtraLesson> getLesList() {
        ArrayList<ExtraLesson> lessonList = new ArrayList<>();
    ...
    public ArrayList<ExtraLesson> getLessonByPage(ArrayList<ExtraLesson> list, int start, int end) {
        ArrayList<ExtraLesson> arr = new ArrayList<>();
    ...
    public void addLesson(ExtraLesson l) {
    ...
    public void activeAndDeactive(ExtraLesson lesson) {
    ...
    public ArrayList<ExtraLesson> searchLesson(String searchQuery, String sub, String chap, String status) {
        ArrayList<ExtraLesson> lessonList = new ArrayList<>();
    ...
    public ArrayList<ExtraLesson> searchLessonTest(String searchQuery) {
        ArrayList<ExtraLesson> lessonList = new ArrayList<>();
        String sql = "select l.*,s.id,s.subject_code,s.subject_name,c.title as chapter_name,c.chapter_id,q.quiz_id,q.quiz_name"
                + " from lesson l join subject s on l.subject_id = s.id\n"
                + " LEFT JOIN chapter c on l.chapter_id = c.chapter_id \n"
    ...
    public ExtraLesson getLessonByTitle(String title) {
        String sql = "select l.*,s.id,s.subject_code,s.subject_name,c.title as chapter_name,c.chapter_id,q.quiz_id,q.quiz_name"
                + " from lesson l join subject s on l.subject_id = s.id\n"
                + " LEFT JOIN chapter c on l.chapter_id = c.chapter_id \n"
                + " LEFT JOIN quiz q on q.quiz_id = l.quiz_id\n"
...
//        d.updateLesson("abc1", "17", "Hello World", "hellow.com", "hellow.docx", "2", "11", 88, 0, "125");
        System.out.println(d.getLesList());
    }

    public ExtraLesson getLessonByURL(String url) {
        String sql = "select l.*,s.id,s.subject_code,s.subject_name,c.title as chapter_name,c.chapter_id,q.quiz_id,q.quiz_name"
                + " from lesson l join subject s on l.subject_id = s.id\n"
                + " LEFT JOIN chapter c on l.chapter_id = c.chapter_id \n"
                + " LEFT JOIN quiz q on q.quiz_id = l.quiz_id\n"
    ...
    //su phu xoa cai chapter_id ho em voi vi no la specific chapter -> khum dc update
    // k sao bên fe không cho ch?nh mà nên ve be nó van se k thay doi , doi tim loi k update gi da da
    ...
    public void updateLesson(String title, String chapter_id, String description, String video_link, String file_attached, String lesson_type, String subject_id, int quiz_id, int updated_by, String lesson_id) {
        String sql = "Update Lesson \n"
                ...
                + "where lesson_id = ?; ";
        try {
            //chapter id null dc ak
            PreparedStatement st = connection.prepareStatement(sql);
            st.setString(1, title);
            if(chapter_id.equals("0"))
            st.setNull(2, java.sql.Types.INTEGER );
            else
                st.setInt(2, Integer.parseInt(chapter_id));
            st.setString(3, description.length()>1000? description.substring(0,1000): description);
...
//em dung chung bang? bang nao luu 
//    public static void main(String[] args) {
//        LessonDAO d = new LessonDAO();
//        d.updateLesson("abc", "20", "abcd", "abc.com", "abc.docs", "3", "8", 84, 0, "18");
//    }
    public ExtraLesson getLessonByID(String lesson_id) {
        ResultSet rs;
        String sql = "select l.*,s.id,s.subject_code,s.subject_name,c.title as chapter_name,c.chapter_id,q.quiz_id,q.quiz_name\n"
                + "from lesson l join subject s on l.subject_id = s.id\n"
                + "LEFT JOIN chapter c on l.chapter_id = c.chapter_id\n"
