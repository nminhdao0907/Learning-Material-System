...
public class ExtraLessons extends HttpServlet {
     ...
     * @throws IOException if an I/O error occurs
     */
    ExtraLessonDAO d = new ExtraLessonDAO();
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        ExtraLessons lesson = new ExtraLessons();
    ...
    private void lessonList(HttpServletRequest request, HttpServletResponse response)
        ...
        ArrayList<ExtraLesson> list = d.getLesList();
        ...
        ArrayList<ExtraLesson> listByPage = d.getLessonByPage(list, start, end);
        ...
        request.getRequestDispatcher("/view/trainer/ExtraLessonList.jsp").forward(request, response);
    }
    ...
    private void Sort(HttpServletRequest request, HttpServletResponse response)
        ...
        ArrayList<ExtraLesson> listByPage = new ArrayList();
        //Sorting title
    ...
    private void searchLesson(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        ArrayList<Chapter> chapterList = cd.getChapterList();
        ArrayList<Quiz> quizList = qd.getAllQuizzes();
        ArrayList<ExtraLesson> list = d.searchLesson(searchQuery, subject, chapter, status);
    ...
    public ArrayList<ExtraLesson> SortByComponent(ArrayList<ExtraLesson> list, String type, String asc) {
        if ("title".equals(type)) {
            if ("0".equals(asc)) {
                Collections.sort(list, new Comparator<ExtraLesson>() {
                    public int compare(ExtraLesson o1, ExtraLesson o2) {
    ...
    private void addLesson(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        request.getRequestDispatcher("exLesson?action=list").forward(request, response);
    }
    ...
    private void updateLesson(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        String subjectId = request.getParameter("subject_id");
        String title = request.getParameter("title");
        String chapterId = request.getParameter("chapter_id").split(" ")[0];
    ...
    private void detailLesson(HttpServletRequest request, HttpServletResponse response, String id) throws IOException, ServletException {
        try {
            ...
            ArrayList<Quiz> quizList = qd.getAllQuizzes();
            ExtraLessonDAO d = new ExtraLessonDAO();
            ExtraLesson list = d.getLessonByID(id);
            request.setAttribute("subjectList", subjectList);
            request.setAttribute("chapterList", chapterList);
            request.setAttribute("quizList", quizList);
            request.setAttribute("lesson", list);
            request.getRequestDispatcher("view/trainer/ExtraLessonDetails.jsp").forward(request, response);
        } catch (Exception e) {
    ...
    private void addScreen(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        request.setAttribute("chapterList", chapterList);
        request.setAttribute("quizList", quizList);
        request.getRequestDispatcher("view/trainer/LessonAdd.jsp").forward(request, response);
    }

    private void activeAndDeactivedLesson(HttpServletRequest request, HttpServletResponse response) {
        String page = request.getParameter("page");
        String lesson_id = request.getParameter("lesson_id");
        ExtraLesson lesson = d.getLessonByID(lesson_id);
        HttpSession session = request.getSession();
        User user = (User) session.getAttribute("user");
        int id = 0;
        if (user != null) {
